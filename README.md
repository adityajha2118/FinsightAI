...
**An enterprise fintech analytics platform combining predictive machine learning, generative AI, and agentic AI to solve critical financial sector challenges including churn prevention, compliance monitoring, and customer support operations.**

[🚀 Project Overview](#project-overview) · [💼 Business Problems Solved](#business-problems-solved) · [🏛️ System Architecture](#system-architecture) · [🗄️ Datasets](#datasets-section) · [🧠 Modules](#modules) · [🤖 AI Layers](#generative-ai-layer)
...
FinSight AI simulates the advanced data analytics infrastructure of a modern, top-tier financial institution (similar to American Express or Chase). By leveraging massive datasets, machine learning, generative AI, and autonomous agentic AI, this unified intelligence platform completely modernizes customer lifecycle management.
...
* **Complaint Intelligence**: Utilizing Large Language Models (LLMs) and autonomous agents to resolve customer grievances.
* **Executive Decision Support**: Providing C-suite leadership with a real-time, interactive dashboard of portfolio health.
...
* **How FinSight solves it:** A LangGraph-powered AI agent classifies incoming complaints, detects emotional distress (e.g., "Legal Threat"), predicts the likelihood of escalation, and autonomously routes high-risk cases to priority teams.
...
FinSight AI employs a highly modular, decoupled architecture. Data flows from raw CSVs through Jupyter Notebook pipelines into a central feature store, which is then served by a FastAPI backend to an interactive Streamlit/Power BI presentation layer.
...
graph TD
    ...
    subgraph Jupyter Data Pipeline & ML
        ...
        P --> |LLM Sentiment| NLP[Complaint NLP]
        NLP --> |XGBoost| ESC[Escalation Module]
    end

    subgraph FastAPI Backend & Agents
        SEG --> API
        CHURN --> API
        INACT --> API
        CAMP --> API
        ...
        AGENT[LangGraph Autonomous Agent] --> API
    end
    ...
    subgraph Frontend Visualization
        ...
        API --> UI_P[Live Complaint Agent UI]
    end
```
...
* **Usage:** Powers the NLP Generative AI layer. Analyzed to detect severe emotional distress and predict regulatory escalation.
...
Traditional NLP (like VADER sentiment analysis) is insufficient for complex financial complaints because it cannot distinguish between frustration over a delayed card and a credible legal threat regarding identity theft. 

FinSight AI integrates **LLMs (Gemini / GPT-4o)** via LangChain to deeply understand unstructured `consumer_complaint_narrative` text.
* **Summarization:** Distills 500-word rants into clean 1-2 sentence summaries for agent dashboards.
* **Classification:** Maps raw text to precise financial categories (e.g., "Billing", "Fraud").
* **Emotion Detection:** Extracts the dominant psychological state—specifically hunting for high-risk flags like **"Legal Threat"** or **"Distress"**.

---

## ⚡ AGENTIC AI LAYER

Moving beyond passive analysis, FinSight features an autonomous **Complaint Routing Agent** built on `LangGraph`.

**Workflow (StateGraph Pipeline):**
1. **Input:** Raw customer complaint text enters the state machine.
2. **LLM Nodes:** The agent autonomously calls the Generative AI layer to summarize, classify, and detect emotion.
3. **ML Node:** The state is passed to the XGBoost Escalation model to calculate an exact probability score.
4. **Decision Rule Node:** The agent evaluates the state:
...
5. **Generative Node:** The LLM drafts a highly personalized, empathetic, and legally-safe response tailored to the specific customer issue and priority level.
6. **Output:** A fully populated JSON object ready for the customer support dashboard.
...
5. **AI Processing:** Massive batch processing of text via LLM APIs saves state to `data/processed/`.
6. **API Serving:** The FastAPI backend loads the models in memory and exposes RESTful endpoints.
7. **Agent Decision Making:** Live requests trigger the LangGraph orchestration loop.
8. **Dashboard Visualization:** Streamlit polls the APIs and renders interactive Plotly visualizations.
...
* **Transformers & Sentence Transformers**: Underpinning architecture for advanced NLP tasks.
* **OpenAI / Google Gemini**: Foundational Large Language Models for the Generative AI layer.
* **LangGraph**: Framework for building stateful, multi-actor LLM applications and agents.
* **FastAPI**: Asynchronous web framework for exposing high-throughput REST APIs.
* **Streamlit / Power BI**: Interactive frontend and enterprise reporting.
* **Plotly**: Advanced, interactive JavaScript-based graphing library.
...
```
FinSight-AI/
├── 📓 notebooks/                  # Step-by-step EDA, ML training, and GenAI processing
...
│   ├── agents/                    # LangGraph state machines and orchestration
...
│   ├── genai/                     # LLM clients, chains, prompts, evaluators
│   ├── pipelines/                 # Domain pipeline orchestrators
│   └── utils/                     # Data/model loaders and validators
├── 🌐 frontend/                   # Streamlit UI application
│   ├── dashboard.py               # Application entry point
│   ├── components/                # Reusable UI components (KPI cards, badges)
│   ├── pages/                     # Individual analytic domain dashboards
│   └── utils/                     # Shared CSS and styling
├── 📊 data/
...
├── 🤖 agents/                     # Agent definitions (prompts, tools, workflows)
├── 📋 prompts/                    # LLM prompt templates
├── ⚙️ configs/                    # YAML configuration files
├── 📊 dashboards/                 # Dashboard specifications by domain
...
* `11_complaint_sentiment.ipynb`: Iterates over complaints with an LLM to generate summaries, classifications, and emotions.
...
2. **Customer Intelligence**: Deep dives into churn probability distributions, PCA scatter plots of segments, and dynamic future-churn watchlists.
...
5. **Complaint Intelligence**: Showcases the AI's NLP capabilities, mapping emotions against complaint categories, and features the **Live AI Agent** interface for real-time complaint routing and response generation.
...
* Transitioned from reactive customer support to predictive, AI-driven escalation prevention.
...
* Expanding the LangGraph agent to autonomously execute database queries for deeper contextual responses.
* Adding a PostgreSQL persistence layer via SQLAlchemy for production-scale data storage.
